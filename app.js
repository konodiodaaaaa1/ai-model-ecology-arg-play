...
function renderRelayAdmin(state) {
  ...
  const monitorPage = `<div class="relay-monitor-page"><section class="relay-monitor-toolbar"><div><strong>渠道监控</strong><small>最近一次探针与上游响应汇总</small></div><div class="relay-monitor-actions"><button>刷新监控</button><button>导出当前视图</button></div></section><section class="relay-monitor-summary"><article><span>在线渠道</span><strong>4 / 6</strong><small>1 个降级 · 1 个归档</small></article><article><span>监控批次</span><strong>RR-0719</strong><small>03:18:02 完成</small></article><article><span>共同字段</span><strong>continuity</strong><small>跨 6 个节点出现</small></article><article><span>待核对</span><strong>1</strong><small>Kemy / Groke 尾段差异</small></article></section><section class="relay-monitor-card"><header><div><strong>最近监控批次</strong><span>六个上游渠道 · 响应与字段健康度</span></div><button class="quiet-button">查看历史批次</button></header><div class="relay-channel-cards">${vendorRows.map((row, index) => `<article class="relay-channel-monitor-card"><header><div><span class="channel-dot status-${row.status}"></span><strong>${row.name}</strong><small>${row.domain}</small></div><span class="relay-status status-${row.status}">${row.status}</span></header><dl><div><dt>请求</dt><dd>R17-${["GR-44","GM-27","GL-09","KM-31","DP-18","LN-52"][index]}</dd></div><div><dt>响应</dt><dd>${["200","retry","200","200","review","archived"][index]}</dd></div><div><dt>字段</dt><dd>${index === 3 ? "operator / tag 缺失" : index === 0 ? "tag=0317" : "continuity"}</dd></div></dl></article>`).join("")}</div><section class="relay-monitor-reconciliation"><div><strong>RR-0719 · 六节点对账</strong><span>六条请求共享 `continuity`；Kemy 行与 Groke raw 流存在末段差异。对账来源已挂接到各渠道监控卡。</span></div><button class="quiet-button" data-relay-monitor-detail>展开对账详情</button></section>${state.relayMonitorDetailOpen ? `<section class="relay-monitor-detail"><header><strong>字段关系 / 来源覆盖</strong><button class="quiet-button" data-relay-monitor-detail>收起</button></header><div class="relay-monitor-detail-grid"><div><span>共同字段</span><code>continuity</code><small>六节点均出现，供应商 schema 未声明 owner</small></div><div><span>冲突字段</span><code>operator / tag</code><small>Kemy 回放保留顺序，Groke raw 流保留 0317 尾段</small></div><div><span>下一步</span><code>回到原生客户端</code><small>具体证据仍需在 Kemy、Groke 与公共仓库中分别核对</small></div></div></section>` : ""}</section></div>`;
