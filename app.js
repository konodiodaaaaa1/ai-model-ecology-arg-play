...
function renderRelayAdmin(state) {
  ...
  const nav = navItems.map((label, index) => `<button class="${(index === 0 && activeSection === "overview") || (index === 7 && activeSection === "audit") ? "active" : ""}" ${index === 0 ? 'data-relay-admin-section="overview"' : index === 7 ? 'data-relay-admin-section="audit"' : ""}><span>${["▦","⌁","◉","◎","▤","⌘","▥","◇","⚙"][index]}</span>${label}</button>`).join("");
  ...
  return `<div class="relay-admin-page"><aside class="relay-admin-sidebar"><div class="relay-admin-brand"><span>R</span><div><strong>Relay</strong><small>管理控制台</small></div></div><nav>${nav}</nav><footer><span class="relay-operator">K2</span><div><strong>room17</strong><small>系统管理员</small></div><button>⋮</button></footer></aside><main class="relay-admin-main"><header class="relay-admin-topbar"><div><h2>${activeSection === "audit" ? "审计日志" : "概览"}</h2><small>Relay Node 17 / production</small></div><div><span class="relay-health-dot"></span>服务运行中<button>通知</button></div></header>${activeSection === "audit" ? auditPage : overview}</main></div>`;
}
